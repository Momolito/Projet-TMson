...
titreA = Label(rootA, text="Bienvenue dans notre programme !", bg="black", fg ="white" , font= ("times new roman", "27"), height=1, width=30)
...
ana_text = StringVar()
ana_btn = Button(zoneOption, textvariable=ana_text, command=lambda: ana_btn_fonction(), bg="#e74c3c", height=2, width=22, anchor=CENTER)
ana_text.set("Analyser un son")
ana_btn.grid(column=2, row=4, padx=15, pady=10)

# Bouton génération Équation
eq_text = StringVar()
eq_btn = Button(zoneOption, textvariable=eq_text, command=lambda: eq_btn_fonction(), bg="#8e44ad", height=2, width=22, anchor=CENTER)
eq_text.set("Génerer un son avec une \n équation mathématique")
eq_btn.grid(column=1, row=2, padx=15, pady=10)

# Bouton génération d'un son Spectre
sp_text = StringVar()
sp_btn = Button(zoneOption, textvariable=sp_text, command=lambda: sp_btn_fonction(), bg="#8e44ad", height=2, width=22, anchor=CENTER)
sp_text.set("Génerer un son à l'aide \n d'un spectre")
sp_btn.grid(column=3, row=2, padx=15, pady=10)


def animateA():
    color1 = ["#e6b0aa", "#fadbd8", "#ebdef0", "#e8daef", "#d4e6f1", "#d6eaf8", "#d1f2eb", "#d0ece7", "#d4efdf", "#d4efdf", "#fcf3cf", "#fdebd0", "#fae5d3", "#f6ddcc", "#fbfcfc", "#f2f3f4", "#eaeded", "#e5e8e8", "#d6dbdf", "#d5d8dc", "#aed6f1" ] #Turquoi: ["#e8f8f5", "#d1f2eb", "#a3e4d7", "#76d7c4", "#48c9b0", "#1abc9c", "#17a589", "#148f77", "#117864", "#0e6251" ] jaune:["#ebf5fb", "#d6eaf8", "#aed6f1", "#85c1e9", "#5dade2", "#3498db"] blue:["#fef5e7", "#fdebd0", "#fad7a0", "#f8c471", "#f5b041", "#f39c12"]
...
def ana_btn_fonction():
    ...
    def return_btn_fonction():
        rootB.destroy()
        os.system("python UpdateProg.py")
        exit()
    ...
    nbHarmGraph_text = Label(rootB, text="Entrez le nombre d'harmoniques à afficher dans le graphique:", anchor="e",width=55)
    ...
    analyse_btn = Button(rootB, textvariable=analyse_text, command=lambda: analyse_btn_fonction(), bg="blue",height=1,width=15)
    analyse_text.set("Analyse!")
    analyse_btn.grid(column=1, row=5)

    # Bouton clear
    clear_text = StringVar()
    clear_btn = Button(rootB, textvariable=clear_text, command=lambda: clear_btn_fonction(), bg="blue", height=1, width=15)
    clear_text.set("Effacer")
    clear_btn.grid(column=1, row=6)

    # Bouton Retout
    return_text = StringVar()
    return_btn = Button(rootB, textvariable=return_text, command=lambda: return_btn_fonction(), bg="gray", height=1, width=15)
...
def sp_btn_fonction():
    ...
    def ConctructionSon(f, E, t):
        ...
        while t < duree:  # ****************************Première partie**************************************************
            valeur = int(
                (float(w1.get()) * math.sin(2 * math.pi * F * t + constantePhase) +  # Calcul des harmoniques pour former le son complexe
    ...
    E_text = Label(root, text="Entrez la fréquence d'échantillonage désirée (par défault 44100 Hz):", anchor="e", width=55)  # Label de la f.Échantillonnage
    ...
    # Bouton analyse
    analyse_text = StringVar()
    analyse_btn = Button(root, textvariable=analyse_text, command=lambda: creation_btn_fonction(), bg="#0df1af", height=1, width=15)
    analyse_text.set("Créer!")
    analyse_btn.grid(column=1, row=9, padx=10, sticky=W)

    # Bouton clear
    clear_text = StringVar()
    clear_btn = Button(root, textvariable=clear_text, command=lambda: clear_btn_fonction(), bg="#0df1af", height=1, width=15)
    clear_text.set("Effacer")
    ...
    w10 = Scale(root, from_=-math.pi, to=math.pi, orient=HORIZONTAL, length=600, tickinterval=(math.pi/6),resolution= (math.pi/6),  activebackground= "cyan", label= "Intervale de -π à π pour la constante de phase:")
    w10.set(0)
    w10.grid(column=0, columnspan=1, row=9, rowspan=1)

    # Bouton Retour
    return_text = StringVar()
    return_btn = Button(root, textvariable=return_text, command=lambda: return_btn_fonction(), bg="gray", height=1, width=15)
    return_text.set("Retour")
...
def eq_btn_fonction():
    rootA.destroy()
    # ********************************************** Classe creationB audio (pour generer un son) **************************************
#Fin
animateA()
rootA.mainloop()